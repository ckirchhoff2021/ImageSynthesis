...
class VGGLoss(nn.Module):
    ...
    def __init__(self, model='vgg16', layer=10, shift=0, reduction='mean', model_file=''):
        super().__init__()
        self.shift = shift
        self.reduction = reduction
        ...
        if len(model_file) == 0:
            ...
            vgg = self.load_local_weights(model_file)
            self.model = vgg.features[:layer+1]

        self.model.eval()
        self.model.requires_grad_(False)

    def load_local_weights(self, model_file):
        vgg = models.vgg16()
...
if __name__ == '__main__':
    ...
    loss = VGGLoss(model_file='/data/juicefs_hz_cv_v3/11145199/pretrained/vgg16-397923af.pth')
    y = loss(x1, x2)
    print(y)
