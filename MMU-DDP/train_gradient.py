...
def _train(model, args):
    ...
    grad_discriminator = PatchDiscriminator(3)
    grad_discriminator = nn.DataParallel(grad_discriminator, device_ids=list(range(torch.cuda.device_count()))).cuda()
    grad_optimizer = torch.optim.Adam(grad_discriminator.parameters(), lr=5e-4, betas=(0.9,0.999))
    grad_optimizer = hvd.DistributedOptimizer(grad_optimizer, named_parameters=grad_discriminator.named_parameters(prefix='grad_discriminator'))
    hvd.broadcast_parameters(grad_discriminator.state_dict(), root_rank=0)
    hvd.broadcast_optimizer_state(grad_optimizer, root_rank=0)
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            loss_grads = criterion(grads_out, grads_gt)
            loss_gradient = loss_gradient + loss_grads
            ...
            freeze(grad_discriminator)
            loss_g_adv = patch_generator_loss(adv_criterion, discriminator, x1, real_label)
            loss_grad_adv = patch_generator_loss(adv_criterion, grad_discriminator, grads_out, real_label)

            loss_tv = tv_criterion(x1)
            loss = loss_content + loss_vgg + loss_fft * 0.1 + (loss_g_adv + loss_grad_adv) * 0.005 + loss_gradient * 0.1 + loss_tv * 0.00000002
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            unfreeze(grad_discriminator)
            ...
            pred_img = model(input_img)
            g1 = model.module.get_gradient(y1)
            _, _, _, _, g2 = pred_img
            loss_grad_d = patch_discrimiantor_loss(adv_criterion, grad_discriminator, g1, g2, real_label, fake_label)
            grad_optimizer.zero_grad()
            loss_grad_d.backward() 
            grad_optimizer.step()
            unfreeze(model)
