...
def train_dataloader(path, count=1e5, batch_size=64, num_workers=0, use_transform=True, distributed=False):
    transform = None
    if use_transform:
    ...
    train_dataset = DeblurFolderDataset(path, count=count, transform=transform)
...
def test_dataloader(path, count=1e5, batch_size=1, num_workers=0, distributed=False):
    test_dataset = DeblurFolderDataset(path, count=count, is_test=True)
...
def valid_dataloader(path, count=1e5, batch_size=1, num_workers=0):
    dataloader = DataLoader(
        DeblurFolderDataset(path, count=count, is_test=True),
        batch_size=batch_size,
        shuffle=False,
        num_workers=num_workers,
        pin_memory=True
...
class DeblurDataset(Dataset):
    def __init__(self, image_dir, data_json, count=1e5, transform=None, is_test=False):
        ...
        print('==> DeblurDataset: image_dir = {}, count = {}'.format(self.image_dir, self.count))
...
class DeblurFolderDataset(Dataset):
    def __init__(self, image_dir, count=1e5, transform=None, is_test=False):
        ...
        print('==> DeblurFolderDataset: image_dir = {}, count = {}'.format(self.image_dir, self.count))
        self.transform = transform
        self.is_test = is_test
